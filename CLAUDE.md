...
- **`chelae trim`** runs the common short-read preprocessing pipeline (poly-G trim → adapter trim → read-structure hard-trim + UMI extraction → poly-X trim → sliding-window quality trim → length/N-base/mean-quality/low-qual-fraction filters) on SE or PE FASTQ data. Input/output may be split files, a single interleaved PE stream (auto-detected on a lone input), or `-` for stdin/stdout. Output compression defaults to `auto` (BGZF for a `.gz`/`.bgz`-suffixed path, plain text otherwise; `--output-compression` forces either encoding regardless of extension). An optional fastp-compatible JSON report is available for MultiQC consumption.
...
- `commands/trim.rs` — The `Trim` subcommand. Large file (~7200 lines) organized into sections per the conventions below. Key types (roughly top-down in the hot path):
  ...
  - `OverlapStats` / `detect_pe_overlap` — PE-overlap evidence acceptance via a single signed-shift outward walk centered on the running mean detected insert; `try_shift_neg` / `try_shift_pos` split the probe by sign so the stats-off hot path skips a runtime branch. With `--insert-size-stats`, the walk extends to positive shifts (the I > R inner-overlap geometry) and emits a fastp-shape histogram in the JSON. 5% hysteresis on center updates. The first acceptable shift is kept only if it's trustworthy (`AcceptedOverlap::trustworthy`: its post-cut tails would match the adapter list by chance with probability ≤ the hidden `--overlap-trust-max-chance`, default 1e-4, via a binomial table on `OverlapAdapterLibrary`; a perfect probe when there are no tails). Otherwise `best_overlap` evaluates every shift and keeps the best by `AcceptedOverlap::better_than` (trustworthy first, then lowest combined mismatch rate, then larger insert), so the result doesn't depend on where the walk started — which is what keeps multi-threaded output deterministic. It can still vary with the walk start in two cases: two different shifts are both trustworthy (adapter-like tails at both), or, with `--insert-size-stats`, a tandem repeat longer than the reads probes perfectly at several tail-less positive shifts, each of which counts as trustworthy. The second changes only the insert-size histogram: with I > R nothing is trimmed. `NegShiftScreen` (in `OverlapScratch`) screens negative shifts 16 at a time on their first 16 probe bases via `screen_16_offsets`: the walk probes shifts within `NEAR_WALK_SHIFTS` of the center directly, then only the screen's survivors in the same order, and `best_overlap` probes only survivors. The screen is a necessary condition for the probe to pass, so output is identical to probing every shift.
  - `AdapterSet` / `Adapter` — the active adapter set (user sequences + kit union + FASTA). `Adapter::pure_acgt` gates the SIMD fast path in `find_adapter_3prime`, which also screens 16 start offsets at a time with `screen_16_offsets` for pure-ACGT adapters of at least 16 bp.
