...
GNU time's verbose format is one `key: value` per line with some keys carrying
units (e.g. `Maximum resident set size (kbytes): 12345`). We keep a curated
subset — the fields most consumers want — and ignore the rest."""
...
def parse(path: str | Path) -> dict:
    ...
    with open(path) as fh:
        for line in fh:
            line = line.rstrip("\n")
            # Split on ": " (colon-space) from the RIGHT — some keys contain
            # their own colons ("Elapsed (wall clock) time (h:mm:ss or m:ss)").
            if ": " not in line:
                continue
